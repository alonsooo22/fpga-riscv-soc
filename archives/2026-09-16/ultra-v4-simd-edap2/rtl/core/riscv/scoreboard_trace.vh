...
// Observation only: keep the profile's semantic source classification accurate.
function [0:0] trace_custom_rr;
    input [31:0] opcode;
    begin
        trace_custom_rr =
          (SUPPORT_XPACK16 && ((opcode & `INST_XPACK16_MASK) == `INST_XPACK16)) ||
          (SUPPORT_XADD16 && ((opcode & `INST_XADD16_MASK) == `INST_XADD16)) ||
          (SUPPORT_MULDIV && SUPPORT_XDOT2H && ((opcode & `INST_XDOT2H_MASK) == `INST_XDOT2H));
    end
endfunction
function [0:0] trace_custom_any;
    input [31:0] opcode;
    begin
        trace_custom_any = trace_custom_rr(opcode) ||
          (SUPPORT_XBEXTU && ((opcode & `INST_XBEXTU_MASK) == `INST_XBEXTU));
    end
endfunction
function [0:0] trace_uses_rs1;
    ...
    begin
        case (opcode[6:0])
        7'h0b: trace_uses_rs1 = trace_custom_any(opcode);
...
function [0:0] trace_uses_rs2;
    input [31:0] opcode;
    begin
        trace_uses_rs2 = trace_custom_rr(opcode) || (opcode[6:0] == 7'h23) ||
                         (opcode[6:0] == 7'h33) ||
                         (opcode[6:0] == 7'h63);
    end
endfunction
...
function [0:0] trace_writes_rd;
    ...
    begin
        case (opcode[6:0])
        7'h0b: trace_writes_rd = trace_custom_any(opcode);
